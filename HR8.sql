...
SELECT * FROM CITY 
WHERE POPULATION > (SELECT POPULATION FROM CITY WHERE NAME = 'Denver');

SELECT NAME, POPULATION 
FROM CITY 
ORDER BY ABS(POPULATION - (SELECT AVG(POPULATION) FROM CITY)) 
LIMIT 1;

SELECT UPPER(NAME) AS city_name_upper
FROM CITY;

SELECT INITCAP(DISTRICT) AS formatted_district
FROM CITY;
...
SELECT NAME, ROUND(POPULATION, -5) AS rounded_population 
FROM CITY;

SELECT NAME, 
       ROUND(POPULATION * 100.0 / (SELECT SUM(POPULATION) FROM CITY), 2) AS percent_of_total 
FROM CITY;
...
GROUP BY DISTRICT 
HAVING COUNT(*) > (
    SELECT COUNT(*) FROM CITY WHERE DISTRICT = 'Texas'
);
